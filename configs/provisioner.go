...
func onlySelfRefs(body hcl.Body) hcl.Diagnostics {
	...
	// that blocks are supported for provisioenrs, we will want to extend this
		...
		for _, v := range attr.Expr.Variables() {
			...
			if !valid {
				diags = append(diags, &hcl.Diagnostic{
					...
					Detail:   "Destroy time provisioners may only reference 'self', `count.index`, or `each.key`",
					Subject:  attr.Expr.Range().Ptr(),
				})
			}
		}
